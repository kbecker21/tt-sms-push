...
import smscenter.smsserver.gateways.AGateway;
import smscenter.smsserver.interfaces.Interface;
import smscenter.smsserver.interfaces.Interface.InterfaceTypes;
...
public class SMSServer {
    ...
    public SMSServer(Properties props) {
        this.props = props;
    ...
    public List<AGateway> getGwList() {
        return this.gwList;
    }
    ...
    private void loadConfiguration() throws Exception {
        ...
        for (int i = 0; i < Integer.MAX_VALUE; i++) {
            try {
                ...
                Class<?> c = Class.forName((gtwClass.indexOf('.') == -1 ? "smscenter.smsserver.gateways." : "") + gtwClass);
        ...
        for (int i = 0; i < Integer.MAX_VALUE; i++) {
            try {
                ...
                Class<?> c = Class.forName((infClass.indexOf('.') == -1 ? "smscenter.smsserver.interfaces." : "") + infClass);
                Constructor<?> constructor = c.getConstructor(argsClass);
    ...
    public void shutdown() throws Exception {
        ...
        Service.getInstance().stopService();
        if (inboundPollingThread != null) {
            inboundPollingThread.interrupt();
            inboundPollingThread.join();
        }
        if (outboundPollingThread != null) {
            outboundPollingThread.interrupt();
            outboundPollingThread.join();
        }
    }
    ...
    void sendMessages() {
        ...
        if (foundOutboundGateway) {
            List<OutboundMessage> msgList = new ArrayList<>();
            try {
                for (Interface<? extends Object> inf : getInfList()) {
                    if (inf.isOutbound()) {
    ...
    public boolean checkPriorityTimeFrame(int priority) {
        ...
        if (priority < 0) {
            timeFrame = getProperties().getProperty("settings.timeframe.low", "0000-2359");
        } else if (priority == 0) {
            timeFrame = getProperties().getProperty("settings.timeframe.normal", "0000-2359");
        } else if (priority >= 0) {
            timeFrame = getProperties().getProperty("settings.timeframe.high", "0000-2359");
        } else {
            timeFrame = "0000-2359";
        }
