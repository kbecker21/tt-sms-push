...
public class RegistrationHandler implements HttpHandler
{
	...
	public RegistrationHandler(Database db)
	{
		this.db = db;
	}
	...
	public void handle(HttpExchange exchange) throws IOException
	{
		...
		try (InputStreamReader reader = new InputStreamReader(exchange.getRequestBody(), StandardCharsets.UTF_8))
		{
			...
			JsonObject keys = subscription.getAsJsonObject("keys");
			String p256dh = keys.get("p256dh").getAsString();
			String auth = keys.get("auth").getAsString();

			db.registerDevice(playerId, endpoint, p256dh, auth);

			JsonObject response = new JsonObject();
			response.addProperty("success", true);
			response.addProperty("message", "Device registered for player " + playerId);
			sendResponse(exchange, 200, gson.toJson(response));
			log.info("Device registered for player {}", playerId);
		}
		catch (Exception e)
		{
